...
def load_dataset_from_text(path_txt, vocab, isLabels = False):
...
#    dataset = dataset.map(lambda tokens: (vocab.lookup(tokens), tf.size(tokens)))
    ...
    if isLabels: # Only return token IDs if labels dataset
...
#                       0))                   # size(tags) -- unused
    ...
    padding_values = ((params.id_pad_word,   # sentence padded on the right with id_pad_word
                   0),                   # size(words) -- unused
                  (params.id_pad_tag))                   # size(tags) -- unused
    dataset = (dataset
